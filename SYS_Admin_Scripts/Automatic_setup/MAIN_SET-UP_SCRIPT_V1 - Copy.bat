...
if "%choice%"=="5" goto update_windows
if "%choice%"=="6" goto clean_balast
...
if exist "%LocalAppData%\Microsoft\WindowsApps\winget.exe" (
    ...
    :: Chrome
    winget install -e --id Google.Chrome --accept-package-agreements --accept-source-agreements --silent --force
    
    :: Mozilla Firefox ESR
    winget install -e --id Mozilla.Firefox.ESR --accept-package-agreements --accept-source-agreements --silent --force
    
    :: GIMP
    winget install -e --id GIMP.GIMP --accept-package-agreements --accept-source-agreements --silent --force
    
    :: Notepad++
    winget install -e --id Notepad++.Notepad++ --accept-package-agreements --accept-source-agreements --silent --force
    
    :: Microsoft Teams (System-wide)
    winget install --id Microsoft.Teams --scope machine --accept-package-agreements --accept-source-agreements --silent
    
    echo.
    echo Do you want to install PDF24 tool? (Y/N)
    set /p choice=Enter choice:  

    if /i "%choice%"=="Y" (
        echo Installing PDF24...
        winget install geeksoftwareGmbH.PDF24Creator --accept-package-agreements --accept-source-agreements --silent --force
    ) else (
        echo Skipping PDF24 installation...
    )
    ...
    echo Attempting to enable it via PowerShell...
    
    powershell -noprofile -executionpolicy bypass -command ^
    "Get-AppxPackage -Name Microsoft.DesktopAppInstaller | Foreach-Object { Add-AppxPackage -DisableDevelopmentMode -Register `"$($_.InstallLocation)\AppxManifest.xml`" }"

    echo.
    echo Checking if WinGet is now available...
    pause
    goto setup_programs_menu
)
...
:: have to use PWSH for winupdate
...
"Try { ^
    ...
    Write-Host 'An error occurred while checking/installing PSWindowsUpdate module:'; ^
    Write-Host $_.Exception.Message; ^
    pause; exit 1; ^
} ^
Write-Host ''; ^
Write-Host 'Available update options:'; ^
Write-Host '1. Show available updates'; ^
Write-Host '2. Install ALL updates'; ^
Write-Host '3. Install Optional Updates (Drivers)'; ^
Write-Host '4. Exit to Main Menu'; ^
$choice = Read-Host 'Enter your choice'; ^
Switch ($choice) { ^
    '1' { Get-WindowsUpdate } ^
    '2' { Install-WindowsUpdate -AcceptAll -AutoReboot } ^
    '3' { Get-WindowsUpdate -MicrosoftUpdate -Category 'Drivers' | Install-WindowsUpdate -AcceptAll -AutoReboot } ^
    '4' { exit 0 } ^
