...
$OutlookProfileExists = Test-Path "$env:USERPROFILE\AppData\Local\Microsoft\Outlook"
...
if ($OutlookProfileExists -eq $true) {
    ...
    $SignatureName = 'BS-Signature-'+$ADUser.l #popravljeno DK
    $SigSource = '\\bs.local\bs\GPO\SCCM\OutlookSignature\Signature - all countries.docx' 
    #$SigSource = 'C:\Users\andrej.kristancic\AppData\Roaming\Microsoft\Signatures\Signature_test.docx'
    #$Sigsource = '\\bs.local\bs\Temp\sani.seraj\Signatures\BS.htm'
    ...
    Function Update-Sig { 
        ...
        if ($MSWord.Selection.Find.Execute($ReplaceText.ToString())) {
           ...
           #SS
           $FindText = $ReplaceText.ToString()
           $Selection = $MSWord.Selection
           $Selection.Find.Execute($FindText, $MatchCase, $MatchWholeWord, $MatchWildcards, $MatchSoundsLike, $MatchAllWordForms, $Forward, $Wrap) > $null
    ...
    #ADUserNumberSplit = $ADUser.TelephoneNumber
    ...
    Update-Sig -attribute "TelephoneNumber" -value "$([string]($ADUser.mobile))" 
    #SS
    Update-Sig -attribute "streetAddress" -value "$([string]($ADUser.streetAddress))"
    Update-Sig -attribute "c" -value "$([string]($ADUser.c))" 
    Update-Sig -attribute "postalCode" -value "$([string]($ADUser.postalCode))" 
    Update-Sig -attribute "l" -value "$([string]($ADUser.l))"
    Update-Sig -attribute "co" -value "$([string]($ADUser.co))" # Državo bo dodalo v ENG jeziku, je ok? #
    Update-Sig -attribute "website" -value "$([string]($ADUser.wWWHomePage))"
    Update-Sig -attribute "company" -value "$([string]($ADUser.company))"
    ...
    # $MSWord.ActiveDocument.SaveAs($path, $wdSaveFormat::wdFormatRTF);
    $saveFormat = [Enum]::Parse([Microsoft.Office.Interop.Word.WdSaveFormat], "wdFormatEncodedText");
    $path = $LocalSignaturePath+'\'+$SignatureName+".txt"
    $default = [Type]::Missing
    $MSWord.ActiveDocument.SaveAs([ref]$path,[ref]$SaveFormat,$default, $default, $default, $default, $default, $default, $default, $default, $default, 65001)
    ...
    Write-host "Cleaning up recent files list in File Explorer.." -ForegroundColor Yellow
    Get-ChildItem -Path "$env:APPDATA\Microsoft\Windows\Recent" -File |  Select-String -Pattern "Sig" | Select-Object -ExcludeProperty path | Remove-Item
    Get-ChildItem -Path "$env:APPDATA\Microsoft\Windows\Recent" -Name *Sig*| Remove-Item
    #Get-ChildItem -Path "$env:APPDATA\Microsoft\Windows\Recent\AutomaticDestinations" -File | Sort-Object LastWriteTime -Descending | Remove-Item
    #>
}
else {
